...
| C13 | Database | Tempat penyimpnan data perangkat lunak  | UC10,UC11,XXXX |
| C14 | LoginUI | Halaman login aplikasi | UC11 |
| C16 | LoginController | Menjebatani LoginUI dengan database | UC11 |
| C17 | ListJasaController | Menghubungkan ListJasaUI dengan Database | UC02|
| C18 | ListJasaUI| Halaman list jasa| UC02 |
| C19 | RegisterUI | Halaman registrasi akun | UC12 |
| C20 | RegisterController | Menjembatani RegisterUI dengan database | UC12 |
| C21 | FormTiket | | |
| C22 | FormTiketUI | Halaman pengajuan tiket | UC08 |
| C23 | FormKontrakUI | Halaman form kontrak pekerjaan| UC05 |
| C24 | FormPekerjaanUI | Halaman detil pekerjaan | UC06 |
| C25 | KontrakPekerjaanController | Mengatur logika kontrak pekerjaan | UC05, UC06, UC07 |
| C26 | KontrakPekerjaanUI | Halaman kontrak pekerjaan | UC07 |
| C27 | FormTiketController | Menjembatani FormTiketUI dengan data FormTiketUI | UC08 |
| C28 | KomunikasiController | Mengatur logika pengiriman pesan | UC03, UC04 |
| C29 | KomunikasiUI | Halaman komunikasi | UC03, UC04 |
| C30 | FormJasaController | Menghubungkan FormJasaUI dengan Database | UC01 |
| C31 | FormJasaUI | Halaman form jasa | UC01 |
| C32 | TiketLaporanUI | Halaman tiketing | UC09 |
| C33 | TiketLaporanController | Mengatur logika tiketing | UC09 |
| C34 | Notifikasi | Mengelola notifikasi perangkat lunak | UC02, UC06, UC08, UC09, UC12 |
...
| C08 | TiketLaporan | Merealisasikan fitur tiket antrian dari laporan yang diberikan Pelanggan dan PemberiJasa |
| C10 | Notifikasi | Merealisasikan fitur notifikasi yang akan diberikan kepada pelapor ketika tiketnya sudah dibalas |
| C21 | FormTiket | Menyimpan data form pengajuan tiket laporan yang diisi pengguna |
| C22 | FormTiketUI | Halaman tampilan form pengisian tiket laporan |
| C27 | FormTiketController | Menjembatani FormTiketUI, FormTiket, TiketLaporan, dan Notifikasi |
| C32 | TiketLaporanUI | Halaman tampilan antrian/daftar tiket laporan |
...
| C08 | TiketLaporan | idTiket, idTugas | +simpanantrean() |
| C10 | Notifikasi | idPengguna, idChat, idNotifikasi, statusNotifikasi, durasiNotifikasi | +kirimNotifikasi() |
| C21 | FormTiket | idTiket, idPengguna, jenisTiket, keteranganTiket, tanggalTiket | - |
| C22 | FormTiketUI | - | +tampilkanTiket() |
| C27 | FormTiketController | databaseHandler | +buatTiket()<br>+balasTiket()<br>+tutupTiket() |
| C32 | TiketLaporanUI | - | +showTiketLaporan() |
