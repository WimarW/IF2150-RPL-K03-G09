<h1>
IF2150 REKAYASA PERANGKAT LUNAK
<br>
TUGAS 2
<br>
REQUIREMENT GATHERING
</h1>
<br>

## *Nama Perangkat Lunak*

### Untuk: Agatha Tatianingseto

Dipersiapkan oleh:

| Informasi | Keterangan |
| --- | --- |
| Kelas | K03 |
| Kelompok | 9  |

| NIM | Nama |
|---|---|
| *13525120* | *Naufal Hasbialhaq* |
| *13525009* | *Wimar Widiarto* |
| *13525093* | *Vinsensius Juan Setiady* |
| *13525126* | *Raymond Edson Sabajan* |
| *13525048* | *Yohanes Nicholas Setiawan* |

---

## Daftar Perubahan

| Revisi | Deskripsi |
| :--- | :--- |
| *A* | *Deskripsikan perubahan yang dilakukan dari dokumen sebelumnya pada dokumen ini. Jika tidak terdapat perubahan, harap kosongkan tabel.* |
| *B* | |
| *C* | |
| ... | |

<br>
<br>

# BAB 1: Deskripsi Umum

## 1.1 Deskripsi Umum Sistem
### 1.1.1 Ekspektasi Pengguna Terhadap Sistem
<p align="justify">Sistem ini melibatkan 3 aktor yang memiliki ekspektasi masing-masing yang berbeda.</p>
<ol>
    <li>Pelanggan: Sistem mampu menyelesaikan masalah yang sedang dialami. </li>
    <li>Pemberi Jasa: Sistem dapat memfasilitasi proses penjualan jasa mereka</li>
    <li>Layanan Pelanggan: Sistem mampu menunjukkan permasalahan yang dialami antar pelanggan dan pemberi jasa sehingga dapat menyelesaikan masalahnya</li>
</ol>

### 1.1.2 Alur kerja Sistem
<ol>
    <li>Pelanggan mengunggah/mendaftarkan informasi kebutuhan jasa ke perangkat lunak </li>
    <li>Aplikasi memberikan rekomendasi pemberi jasa relevan untuk kebutuhan pelanggan </li>
    <li>Pelanggan dan pemberi jasa melakukan negosiasi harga untuk pekerjaan yang ingin dilakukan</li>
    <li>Jika disetujui, pelanggan menginput persetujuan ke dalam aplikasi</li>
    <li>Pemilik jasa melakukan dan menyelesaikan pekerjaan fisik sesuai dengan persetujuan</li>
    <li>Setelah pekerjaan selesai, pelanggan akan melakukan pembayaran sebesar harga yang ditetapkan</li>
    <li>Proses diakhiri dengan kedua pihak saling memberi penilaian terkait kinerja pemberi jasa dan perilaku pelanggan</li>
</ol>

### 1.1.3 Harapan Penerapan Solusi
<p align="justify">Penerapan sistem ini diharapkan dapat menjaga standar kualitas dan profesionalisme penggunanya melalui sistem penilaian. Dengan sistem ini, pihak pelanggan dan pemberi jasa diwajibkan memberikan pelayanan dan perilaku yang terbaik untuk menjaga penilaian mereka. Sistem juga diharapkan dapat memberikan rasa aman dengan menyediakan lyananan ticket aduan apabila terdapat keluhan yang tidak bisa diselesaikan antara kedua belah pihak.</p>


## 1.2 Deskripsi Pengguna Perangkat Lunak

Buatlah daftar seluruh aktor (pengguna) yang akan berinteraksi langsung dengan sistem solusi yang kalian kembangkan. Berikan penjelasan singkat mengenai peran dan karakteristik dari masing-masing aktor tersebut.

| Aktor | Deskripsi |
| :--- | :--- |
| Pemberi Jasa | Pengguna ini bertindak sebagai pihak penyedia jasa yang menerima pesanan, melakukan pekerjaan fisik di lokasi pelanggan, dan menyelesaikan tugas sesuai dengan persetujuannya dengan pelanggan.  |
|  Pelanggan| Pengguna ini berperan sebagai pihak yang memerlukan, memesan, dan membayar layanan jasa kasar. |
| Layanan Pelanggan | Pengguna ini sebagai pihak yang berjaga jaga apabila terdapat sebuah masalah pada sistem atau masalah pada pengguna lain |

---

# BAB 2: Deskripsi Kebutuhan Perangkat Lunak

## 2.1 Kebutuhan Pengguna Awal

Definisikan apa yang ingin dicapai oleh pengguna saat menggunakan sistem ini dalam format *User Story* (Sebagai [Aktor], saya ingin [Aktivitas/Kebutuhan], sehingga [Tujuan/Nilai]). Pastikan kalian berfokus pada "apa yang ingin dilakukan pengguna".

| ID | Aktor | Kebutuhan / Aktivitas | Tujuan / Nilai |
| :--- | :--- | :--- | :--- |
| US-01 | Pemberi Jasa |  Melakukan pekerjaan fisik di lokasi pelanggan sesuai dengan persetujuan dengan pelanggan  | Menjual jasa untuk mendapatkan upah  |
| US-02 | Pelanggan | Pihak yang memerlukan, memesan, dan membayar layanan jasa | Menyelesaikan masalah yang dimiliki oleh pelanggan|
| US-03 | Layanan Pelanggan | menyelesaikan permasalahan yang terjadi dalam aktivitas pemberi dan penerimaan jasa | Menyelesaikan permasalahan yang terjadi antara pihak Pemberi Jasa dan Pelanggan |

## 2.2 Deskripsi Aktivitas

Buatlah daftar seluruh aktivitas yang terdapat dalam sistem solusi, lengkap dengan ID dan penjelasan. Telusuri hubungan aktivitas tersebut dengan *user story* yang sudah dituliskan sebelumnya. Bisa dibuat dalam bentuk tabel.

| ID | Aktivitas | Penjelasan | ID User Story |
| :--- | :--- | :--- | :--- |
| A01 | Mendaftarkan Kebutuhan Jasa | Pelanggan memasukkan informasi pekerjaan. | US-01 |
| A02 | Mengambil Pekerjaan | Pemberi jasa mendaftarkan diri untuk kebutuhan jasa pelanggan | US-02 |
| A03 | Negosiasi Harga| pelanggan dan pemberi jasa bernegosiasi harga dari tingkat intensitas pekerjaan yang dilakukan | US-01 US-02 |
| A04 | Input Persetujuan Pekerjaan | Pelanggan memasukkan persetujuan pekerjaan dan harga yang akan dibayarkan | US-01 |
| A05 | Menyelesaikan Pekerjaan | Pemberi jasa melaporkan selesainya pekerjaan yang dilakukan | US-02 |
| A06 | Rating | Pelanggan memberikan rating akan pekerjaan yang dilakukan pemberi jasa dan sebaliknya| US-01 US-02 |

## 2.3 Pemetaan Kebutuhan

Perhatikan kembali semua aktivitas yang telah didefinisikan pada tabel deskripsi aktivitas atau *activity diagram*. Jabarkan kebutuhan sistem yang akan dibuat dengan mengacu pada aktivitas-aktivitas tersebut. Setiap aktivitas (ID Aktivitas) dapat memiliki satu atau lebih kebutuhan yang berbeda. Pastikan untuk mengidentifikasi dan mengisi semua jenis kebutuhan yang relevan untuk setiap aktivitas, yaitu:

- **User Requirement**, yaitu kebutuhan dari sudut pandang pengguna (apa yang dapat dilakukan pengguna).
- **Business Requirement**, yaitu aturan, kebijakan, atau standar bisnis yang harus dipenuhi oleh sistem.
- **System Requirement**, yaitu kebutuhan yang menjelaskan apa yang harus dilakukan sistem dan bagaimana sistem harus bekerja dari segi performa, keamanan, keandalan, dsb.

Lengkapi juga dengan penjelasannya dan apakah keperluan tersebut perlu didukung oleh perangkat lunak atau tidak. Jenis kebutuhan tidak terbatas hanya dari tiga jenis di atas, dapat ditambahkan yang lain juga bila diperlukan, misalnya kebutuhan regulasi (*Legal*).

A01 :
-User : Upload kebutuhan jasa
-Bussines : Jasa nya di sekitar sektor informal
-System : Sistem menyimpan data kebutuhannya

A02 : 
-UR : Pemberi jasa mendaftarkan diri jasa yang diberikannya 
-BR : Pemberi jasa mengaktifkan gps/lokasi
-SR : Sistem mencatat titik lokasi dari pemberi jasa

A03 : 
-UR : Pelanggan dan pemberi jasa berkomunikasi untuk estimasi harga
-BR : Negosiasi harga dilakukan tanpa tekanan dari pihak lain
-SR : Sistem memberi platform chat atau kontak dari pelanggan atau pemberi jasa

A04 : 
-UR : Pelanggan memberikan deskripsi jasa dan memberi estimasi harga
-BR : Pekerjaan dilaksanakan ketika kedua belah pihak menyetujui harga 
-SR : Sistem mengunci kesepakatan ketika kedua belah pihak sudah setuju

A05 :
-UR : Pemberi jasa mengonfirmasi selesainya pekerjaan, pelanggan membayar
-BR : Pelanggan wajib membayar
-SR : Sistem memperbarui status pesanan, mencatat riwayat transaksi

A06 :
-UR : Pelanggan dan pemberi jasa memberi memberi rating
-BR : Penilaian bersifat dua arah, (mutual)
-SR : Sistem mengalkulasi rata-rata rating secara berkala


| ID Kebutuhan | ID Aktivitas | Jenis Kebutuhan | Deskripsi Kebutuhan | P/L |
| :--- | :--- | :--- | :--- | :--- |
| *R01* | *A01* | *User* | *Pengguna dapat memilih metode pembayaran dan melakukan pembayaran secara digital.* | *Ya* |
| *R02* | *A01* | *Business* | *Transaksi digital sesuai dengan ketentuan UU ITE yang berlaku.* | *Tidak* |
| *R03* | *A01* | *System* | *Sistem harus mengintegrasikan API Payment Gateway dengan prinsip ACID (Atomicity, Consistency, Isolation, Durability), jika terjadi kegagalan jaringan saat saldo terpotong, sistem harus secara otomatis membatalkan transaksi atau meneruskan dana (reliable).* | *Ya* |
| *R04* | *A01* | *System* | *Kata sandi (password) atau PIN pengguna saat otorisasi pembayaran harus di-hash menggunakan algoritma SHA-256 dan tidak disimpan dalam bentuk plain-text.* | *Ya* |
| *R05* | *A02* | *Business* | *Toko harus memiliki rekening bank aktif dan valid untuk menerima pencairan dana dari sistem.* | *Tidak* |
| ... | ... | ... | ... | ... |

## 2.4 Kebutuhan Fungsional (KF)

| ID KF | ID Kebutuhan | Penjelasan |
| :--- | :--- | :--- |
## 2.3 Kebutuhan Fungsional

| ID KF | ID Kebutuhan | Penjelasan |
| :--- | :--- | :--- |
| KF-01 | R01 | Sistem harus menyediakan form untuk mengunggah kebutuhan jasa yang memungkinkan pelanggan mengisi deskripsi pekerjaan, kategori jasa, dan lokasi yang dibutuhkan |
| KF-02 | R01 | Sistem harus menyimpan data kebutuhan jasa pelanggan ke dalam basis data dan menampilkannya kepada pemberi jasa yang relevan berdasarkan kategori dan lokasi |
| KF-03 | R02 | Sistem hanya menampilkan dan menerima kategori jasa yang termasuk dalam sektor informal (misalnya tukang, ojek, cleaning service, dll.), dan menolak jasa di luar cakupan tersebut |
| KF-04 | R03 | Sistem meminta izin akses GPS/lokasi kepada pemberi jasa saat pendaftaran dan secara berkala memperbarui titik koordinat lokasi pemberi jasa yang sedang aktif |
| KF-05 | R03 | Sistem harus mencatat dan menyimpan data titik lokasi pemberi jasa secara real-time ke dalam basis data untuk digunakan pada fitur pencarian berbasis lokasi |
| KF-06 | R04 | Sistem harus menyediakan formulir pendaftaran bagi pemberi jasa yang mencakup data diri, jenis jasa yang ditawarkan, dan nomor rekening aktif untuk pencairan dana |
| KF-07 | R05 | Sistem harus memvalidasi kelengkapan dan keaktifan rekening bank yang didaftarkan pemberi jasa sebelum akun disetujui dan dapat digunakan |
| KF-08 | R06 | Sistem harus menyediakan fitur platform chat antara pelanggan dan pemberi jasa untuk proses negosiasi harga dan diskusi detail pekerjaan |
| KF-09 | R06 | Sistem harus menampilkan informasi kontak (misalnya nomor telepon/WhatsApp) pemberi jasa kepada pelanggan sebagai alternatif komunikasi di luar platform |
| KF-10 | R07 | Sistem tidak menyertakan pihak ketiga manapun dalam sesi percakapan antara pelanggan dan pemberi jasa, sehingga negosiasi bersifat privat dan tanpa tekanan eksternal |
| KF-11 | R08 | Sistem harus menyediakan form bagi pelanggan untuk mengisi deskripsi rinci pekerjaan dan mengajukan penawaran harga awal sebelum pekerjaan dimulai |
| KF-12 | R09 | Sistem harus menampilkan status kesepakatan (menunggu/disetujui/ditolak) kepada kedua belah pihak dan mengunci detail pekerjaan serta harga apabila kedua pihak telah menyatakan setuju |
| KF-13 | R09 | Sistem mencegah perubahan deskripsi pekerjaan atau harga setelah status kesepakatan terkunci, kecuali melalui proses renegosiasi yang dimulai ulang oleh salah satu pihak |
| KF-14 | R10 | Sistem harus menyediakan tombol konfirmasi penyelesaian pekerjaan bagi pemberi jasa, dan sistem mengirimkan notifikasi kepada pelanggan untuk melakukan pembayaran |
| KF-15 | R11 | Sistem mewajibkan pelanggan menyelesaikan pembayaran sebelum status pesanan dapat berubah menjadi "Selesai" |
| KF-16 | R12 | Sistem harus memperbarui status pesanan secara otomatis dan mencatat seluruh riwayat penggunaan jasa (waktu, nominal, pihak terlibat) ke dalam basis data setelah pembayaran berhasil |
| KF-17 | R13 | Sistem harus menyediakan form penilaian (rating bintang dan ulasan teks) bagi pelanggan untuk menilai pemberi jasa dan sebaliknya, setelah status pesanan dinyatakan selesai |
| KF-18 | R14 | Sistem harus memastikan kedua pihak (pelanggan dan pemberi jasa) hanya dapat memberikan penilaian satu kali per transaksi yang telah selesai |
| KF-19 | R15 | Sistem harus menghitung dan memperbarui rata-rata rating profil pemberi jasa dan pelanggan secara otomatis setiap kali penilaian baru diterima |

## 2.5 Kebutuhan Non-Fungsional (KNF)

| ID KNF | ID Kebutuhan | Parameter | Deskripsi Kebutuhan |
| :--- | :--- | :--- | :--- |
| *KNF01* | *R03* | *Reliability* | *Proses transaksi pembayaran harus memenuhi prinsip ACID untuk mencegah terjadinya data tersangkut (lost update) apabila terjadi kegagalan jaringan di tengah proses.* |
| *KNF02* | *R04* | *Security* | *Sistem harus mengenkripsi PIN atau password pengguna menggunakan algoritma SHA-256 sebelum data dikirimkan ke server, serta tidak menyimpannya dalam bentuk plain-text di database.* |
| *KNF03* | ... | *Security* | *Ketika user mengunggah data penting pada formulir, sistem harus memvalidasi input formulir oleh user pada sisi server untuk mencegah adanya serangan pada celah keamanan seperti SQL Injection* |
| *KNF04* | ... | *Response time* | *Ketika user menggunggah data penting pada formulir, sistem harus memvalidasi input user pada formulir harus tidak melebihi 2 detik dalam kondisi internet normal* |
| *KNF05* | ... | *Response time* | *Ketika user membuka peta, sistem harus dapat menampilkan daftar pemilik jasa kepada pengguna jasa dalam waktu 2 detik pada keadaan koneksi internet normal* |
| *KNF06* | ... | *Reliability* | *Ketika user membuka peta, GPS harus dapat ditampilkan dengan akurat pada pengguna jasa hingga < radius 50 meter* |
| *KNF07* | ... | *Security* | *Ketika user atau pemilik jasa mengirim pesan, pesan yang dikirim harus dienkripsi dengan End-to-End Encryption* |
| *KNF08* | ... | *Response time* | *Ketika user atau pemilik jasa mengirim pesan, pesan yang dikirim harus dapat sampai ke pihak yang menerima pesan dalam waktu 1 detik pada keadaan koneksi internet normal* |
| *KNF09* | ... | *Reliability* | *Selama sistem pengiriman pesan aktif, sistem harus dapat menangani hingga 1000 percakapan serentak dari semua pengguna tanpa adanya masalah dalam komunikasi pesan pada keadaan koneksi internet normal* |

MD
System Requirement (SR):
Sistem harus mengelola state (status) transaksi dari draf persetujuan menjadi disetujui atau ditolak. 

| *KNF10* | ... | *Reliability* | 

Silakan pilih yang relevan. Tidak perlu semua parameter menjadi kebutuhan non-fungsional. Berikut merupakan penjelasan dari setiap parameter. **Parameter dari Kebutuhan Non-Fungsional tidak terbatas hanya di bawah ini** karena hanya merupakan panduan sehingga dapat ditambah KNF yang lain, misalnya *constraint* dari sistem.

| Parameter | Penjelasan |
| :--- | :--- |
| *Availability* | Ketersediaan aplikasi, misalnya harus terus-menerus beroperasi 7 hari per minggu, 24 jam per hari tanpa gagal. |
| *Reliability* | Keandalan, misalnya tidak pernah boleh gagal (atau kegagalan yang ditolerir adalah …%) sehingga harus dipikirkan *fault tolerant architecture*. Biasanya hanya perlu untuk *critical application* yang jika gagal akan berakibat fatal. |
| *Ergonomy* | Kenyamanan pakai bagi pengguna. |
| *Portability* | Kemudahan untuk dibawa dan dioperasikan ke mesin/sistem operasi/*platform* yang lain. |
| *Memory* | Jika perhitungan kapasitas memori internal kritis (misalnya untuk P/L yang harus dijadikan *chips* dan ukurannya harus kecil). |
| *Response time* | Batasan waktu yang harus dipenuhi. Sangat penting untuk aplikasi *real time*. Contoh: "Aplikasi harus mampu menampilkan hasil dalam 4 detik", atau "ATM harus menarik kembali kartu yang tidak diambil dalam waktu 3 menit". |
| *Safety* | Yang menyangkut keselamatan manusia, misalnya untuk P/L yang dipakai pada sistem kontrol di pabrik. |
| *Security* | Aspek keamanan yang harus dipenuhi. |

<br>

# Referensi
- Diagram UML: https://www.drawio.com/, https://staruml.io/
